...
#[derive(Accounts)]
#[instruction()]
pub struct ChargeFeeAndSendNftBack<'info> {
...
pub fn charge_fee_and_send_nft_back(ctx: Context<ChargeFeeAndSendNftBack>) -> Result<()> {
    // Procedd to renting nft
    let cpi_program = ctx.accounts.token_program.to_account_info();
    let cpi_accounts = Transfer {
        from: ctx.accounts.pda_rent_account.to_account_info(),
